...
public class BulletinBoardRepositoryImpl implements BulletinBoardRepository {
    ...
    public void removeBoardById(String board_id) {
        dbManager.remove(dbManager.findById(new Query(Criteria.where("id")
                .is(board_id)), BulletinBoard.class));
    }

    @Override
    public void removeAllStickersFromBoard(String board_id) {
        dbManager.remove(new Query(Criteria.where("board_id").is(board_id)), Sticker.class);
    }
}
